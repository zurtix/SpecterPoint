...
pub async fn get_server(pool: SqlitePool, id: &i64) -> Result<Server> {
    Ok(sqlx::query_as(r#"SELECT id, name, type, host, port, log_port, username, password FROM servers WHERE id = ?1"#)
...
pub async fn get_server_ids(pool: SqlitePool) -> Result<Vec<i64>> {
    Ok(sqlx::query_scalar(r#"SELECT id FROM servers"#)
        .fetch_all(&pool)
        .await?)
}
...
pub async fn create_server_listeners(
    ...
    sqlx::query(r#"INSERT INTO server_listeners (listener_id, server_id) VALUES (?1, ?2)"#)
        .bind(listener_id)
        .bind(server_id)
        .execute(&pool)
        .await?;
